<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>

<!DOCTYPE HTML PUBLIC>
<html>
<head>
<meta name="robots" content="noindex">
<meta charset="UTF-8">
<meta name="viewport"
	content="width=device-width,initial-scale=1.0,minimum-scale=1.0,maximum-scale=1.0,user-scalable=0">
<link rel="stylesheet" href="css/style.css" type="text/css"
	media="screen and (min-width: 640px)">
<link rel="stylesheet" href="css/account.css" type="text/css"
	media="screen and (min-width: 640px)">
<link rel="stylesheet" href="css/featherlight.css"
	media="screen and (min-width: 640px)">
<link rel="stylesheet" href="css/style_sp.css" type="text/css"
	media="screen and (max-width: 640px)">
<link rel="stylesheet" href="css/account_sp.css" type="text/css"
	media="screen and (max-width: 640px)">
<link rel="stylesheet" href="css/featherlight_sp.css"
	media="screen and (max-width: 640px)">
<script type="text/javascript" charset="UTF-8" src="js/change.js"></script>
<script type="text/javascript"
	src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
<script type="text/javascript" src="js/featherlight.js"></script>
<script type="text/javascript">
	$(function() {
		$(".l_header").load("inclu/header2.jsp");
		$(".l_footer").load("inclu/footer.jsp");
	});
</script>
<title>RMS|新入社員管理システム</title>
</head>
<body>
	<header id="global-header">
		<div id="head-inner">
			<div class="l_header"></div>
		</div>
		<div id="menu-bar"></div>
	</header>
	<div id="scrollframe">
		<br />
		<p id="headtitle">パスワード再設定</p>
		<h3>下記情報を全て入力し、パスワードの再設定を行ってください。</h3>
		<form name="defaultLink2" action="mail.do" method="post">
			<input type="hidden" name="action" />
			<c:if test="${errorList != NULL}" var="flg">
				<table class="errbox">
					<tr>
						<td><b>◆入力内容に誤りがあります。</b></td>
					</tr>
				</table>
				<table class="errbox2">
					<c:forEach var="error" items="${errorList}">
						<tr>
							<td><p class="err">&nbsp;・ ${error}</p></td>
						</tr>
					</c:forEach>
				</table>
			</c:if>
			<table class="search">
				<tr>
					<th class="paragraphbox">ID</th>
					<td class="paragraphbox"><input type="text"
						placeholder="例) SQT001" name="accountId" maxlength="30" size="40"
						value="${accountId}" required title="IDは30文字まで入力可能です。" /></td>
				</tr>
				<tr>
					<th class="paragraphbox">新しいパスワード</th>
					<td class="paragraphbox"><input type="password"
						placeholder="例) sqt3666" name="accountPassword" maxlength="8"
						size="20" value="${Password}" required
						title="パスワードは8文字まで入力可能です。" /></td>
				</tr>
				<tr>
					<th class="paragraphbox">新しいパスワード再入力</th>
					<td class="paragraphbox"><input type="password"
						placeholder="例) sqt3666" name="accountRePassword" maxlength="8"
						size="20" value="${RePassword}" required
						title="パスワードは8文字まで入力可能です。" /></td>
				</tr>
			</table>
			<table class="button2">
				<tr>
					<td>
						<p style="position: relative;">
							<input type="image" src="img/send.png"
								onclick="goSubmit(this.form, this);" name="PassReset_btn"
								value="" alt="再設定">
						</p>
					</td>
				</tr>
			</table>
		</form>
	</div>
	<div class="l_footer"></div>
</body>
</html>